...
struct SharePayloadSheet: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $isActivityPresented) {
            ActivitySheet(items: activityItems)
        }
    }
    ...
    private var qrCode: some View {
        ZStack {
            if let qrImage {
                ...
                LegacyUnavailableView("无法生成二维码", systemImage: "qrcode")
            } else {
                VStack(spacing: 12) {
                    ProgressView()
                    Text("正在生成二维码")
